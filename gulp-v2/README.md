...
Здесь лежат кнопки, чекбоксы, хлебные крошки, селектменю, модалки.
...
Также сюда можно выносить такие файлы как `btn`, `form`, `modal` в которых нет миксинов, а значит и pug-файлов:
```bash
btn/        # Папка блока
...
Каждый блок в `src/components/` может содержать одноимённый pug-файл с примесью, который при старте сервера разработки будет взят includ-ом в файл `src/pug/mixins.pug`.
